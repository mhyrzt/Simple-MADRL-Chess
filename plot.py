...
def plot_ma(ax, arr, episodes = -1, count: int = 50):
    for i in range(2):
        c = W if i else B
        ax.plot(ma(arr[i, :episodes], count), c=c, alpha=1)
    return ax
...
def plot_moves(ax, moves, episodes, count: int = 50):
    ...
    ax.plot(ma(arr, count), alpha=1, c=B)
...
def density(arr, count, episode):
    a = arr.max(axis=0)
    zs = []
    for i in range(count, episode):
        j = i - count
        z = np.sum(a[j: i])
        zs.append(z)
    return zs

def plot_check_mates(ax, check_mates_arr: np.ndarray, episodes: int, count_density: int):
    density_ax = ax.twinx()
...
moves = np.load("results/moves.npy")
mates = np.load("results/mates_win.npy")
checks = np.load("results/checks_win.npy")
...
plot_ma(axs[0, 0], rewards, episodes, count=COUNT)

plot_moves(axs[0, 1], moves, episodes, count=COUNT)

plot(axs[1, 0], checks, "Checks", episodes, alpha=ALPHA)
plot_ma(axs[1, 0], checks, episodes, count=COUNT)


plot_check_mates(axs[1, 1], mates, episodes, COUNT)

fig.tight_layout()
fig.savefig("results/plots.jpeg")
plt.show()